...
import AddGameForm from '@/admin/Competitions/forms/AddGameForm.vue'
  ...
  const result = Array.isArray(row?.value?.games)
    ? row?.value?.games
        .filter((game: Game) => {
          ...
          const isBeforeNextPhase = nextPhase 
            ? isAfter(nextPhase?.datetime, game.datetime)
            : true
          return isAfterCurrentPhase && isBeforeNextPhase
        })
        .map((game: Game) => {
          return {
            ...game,
