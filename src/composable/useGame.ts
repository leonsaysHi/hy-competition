...
export default function useGame(competitionId: CompetitionId, gameId: GameId) {
  ...
  const { row: competition, teams, games, config } = useCompetition(competitionId)
  ...
  const isReady = computed(() => isLibsReady.value && row.value?.id && competition.value?.id && rosters.value)
