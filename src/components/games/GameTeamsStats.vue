...
const competitionStatsKeys = computed<PlayerStatKey[]>(() => {
  ...
  ? playerStatsKeys
    .filter((opt: Option) => competition.value.trackedStats.includes(opt.value as PlayerRankingKey))
...
const teamsTotalsByStats = computed(() => {
  return props.teams
    .reduce((teams, team: CompetitionTeam) => {
      const teamId: TeamId = team.id
      teams[teamId] = competitionStatsKeys.value
        .reduce((totals, statKey: PlayerStatKey) => {
          const boxscores: PlayerBoxScore[] = team.players
            .map((player: CompetitionPlayer):PlayerBoxScore => {
              const playerId:PlayerId = player.id
              return props.boxscore[playerId]
            })
          totals[statKey] = boxscores
            .reduce((tot: number, bs: PlayerBoxScore) => tot + bs[statKey], 0)
          return totals
        }, {})
      return teams
    }, {})
...
const teamsBarsByStats = computed(() => {
  const result = Object.keys(teamsTotalsByStats.value)
      .reduce((result, teamId: TeamId) => {
        result[teamId] = {}
        return result
      }, {})
  competitionStatsKeys.value
    .forEach((statKey: PlayerStatKey) => {
      ...
      Object.keys(teamsTotalsByStats.value)
        .forEach((teamId: TeamId, idx: number) => {
...
<template>
  ...
  <template v-for="(statKey, idx) in competitionStatsKeys" :key="statKey">
    ...
    <div class="progress-stacked" style="height: 5px;">
      <div class="progress" role="progressbar" aria-label="Segment one" :style="`width: ${teamsBarsByStats[teams[0].id][statKey]}%;`">
        <div class="progress-bar" :class="teamsBarsByStats[teams[0].id][statKey] >= teamsBarsByStats[teams[1].id][statKey] ? 'bg-success' : 'bg-secondary'"></div>
      </div>
      <div class="progress ms-auto" role="progressbar" aria-label="Segment two" aria-valuenow="30" aria-valuemin="0" aria-valuemax="100"  :style="`width: ${teamsBarsByStats[teams[1].id][statKey]}%;`">
        <div class="progress-bar"  :class="teamsBarsByStats[teams[0].id][statKey] <= teamsBarsByStats[teams[1].id][statKey] ? 'bg-success' : 'bg-secondary'"></div>
      </div>
    </div>
  </template>
